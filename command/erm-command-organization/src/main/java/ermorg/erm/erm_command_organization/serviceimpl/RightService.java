...
import ermorg.erm.erm_command_organization.model.history.RightHistory;
...
import ermorg.erm.erm_command_organization.repository.history.RightHistoryRepository;
...
public class RightService implements IRightService {
	...
	public List<RightResponse> getAllRights() {
		List<RightResponse> rights = rightRepository.findAll().stream().filter(r -> !r.getDeleted())
				.map(right -> new RightResponse(right)).collect(Collectors.toList());
	...
	public RightResponse getRightById(long rightId) throws ResourceNotFoundException {
		...
		Right right = rightRepository.findById(rightId).filter(r -> !r.getDeleted())
				.orElseThrow(() -> new ResourceNotFoundException("Right not found."));
		return new RightResponse(right);
	}

	@Override
	public RightResponse saveRight(RightDto request) throws ResourceNotFoundException {
		Modules module = moduleRepository.findById(request.getModuleId()).filter(m -> !m.getDeleted())
