...
public class ApiLoggingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            logger.error("API ERROR method={} uri={}",
                    request.getMethod(),
    ...
    private void logRequest(ContentCachingRequestWrapper request) {
        ...
        if (isBodyAllowed(method)) {
            body = getPayload(request.getContentAsByteArray(), request.getCharacterEncoding());
        }
    ...
    private String getPayload(byte[] content, String encoding) {
        ...
        int length = Math.min(content.length, MAX_PAYLOAD);
