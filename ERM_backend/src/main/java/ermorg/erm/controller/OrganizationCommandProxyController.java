...
/**
 * Proxies read-only organization hierarchy calls to erm-command-organization so
 * clients can use gateway route {@code /erm/**} (StripPrefix) while CQRS data
 * lives in the command service.
 */
@RestController
public class OrganizationCommandProxyController {
	...
	public ResponseEntity<String> getBusinessVerticalsBySegmentId(@PathVariable Long segmentId) {
		String url = String.format("http://%s/business-vertical/get-all-by-segment-id/%d", commandOrganizationServiceId,
				segmentId);
		return exchangeAsJson(url);
	}
